...
export class ProxyConfig {
  ...
  private detectLocalProxy(): void {
    ...
    if (
      this.proxyUrl &&
      !this.proxyUrl.includes("localhost") &&
      !this.proxyUrl.includes("127.0.0.1")
    ) {
      log.info(
        "[ZscalerConfig] Non-localhost proxy already configured, skipping localhost detection"
      );
      return;
    }
    ...
    for (const port of localProxyPorts) {
      ...
      if (process.env[`LOCALHOST_PROXY_${port}`] === "true") {
        ...
        if (port === 8005) {
          log.info(
            "[ZscalerConfig] ⚠️  WSUS/MSDTC proxy detected on port 8005 - Mutual TLS likely required"
          );
          log.info(
            "[ZscalerConfig] This proxy requires special handling for certificate authentication"
          );
        }
    ...
    if (!this.proxyUrl) {
      ...
      log.info(
        "[ZscalerConfig] This often indicates enterprise network with mutual TLS requirements"
      );
    }
  }
  ...
  public logConfiguration(): void {
    ...
    log.info(
      `  - NODE_TLS_REJECT_UNAUTHORIZED: ${process.env.NODE_TLS_REJECT_UNAUTHORIZED || "Default (1)"}`
    );
