 ...
 */
export const TableEditor = forwardRef<TableEditorRef, TableEditorProps>(function TableEditor(
  {
    table,
    tableIndex,
    onChange,
  ...
  const deleteRow = useCallback(() => {
    ...
    newTable.rows = newTable.rows.filter(
      (_, index) => index < selectionBounds!.minRow || index > selectionBounds!.maxRow
    );
  ...
  const deleteColumn = useCallback(() => {
    ...
    newTable.rows = newTable.rows.map((row) => ({
      ...
      cells: row.cells.filter(
        (_, index) => index < selectionBounds!.minCol || index > selectionBounds!.maxCol
      ),
    }));
  ...
  const mergeCells = useCallback(() => {
    ...
    const mergedText = newTable.rows
      ...
      .flatMap((row) =>
        row.cells.slice(minCol, maxCol + 1).flatMap((cell) => cell.paragraphs.map((p) => p.text))
      )
      .filter((t) => t)
      .join(" ");
  ...
  const contextMenuItems: ContextMenuItem[] = [
    ...
    { id: "divider-3", label: "", icon: Plus, action: () => {}, divider: true },
    {
      ...
      icon: Paintbrush,
      action: () => tableShadingSettings && applyShading(tableShadingSettings.header2Shading),
    ...
    },
    {
      ...
      icon: Paintbrush,
      action: () => tableShadingSettings && applyShading(tableShadingSettings.otherShading),
      disabled: !tableShadingSettings,
    },
  ];

  return (
    <div className="relative my-4 overflow-x-auto" onKeyDown={handleKeyDown} tabIndex={0}>
      <table ref={tableRef} className="w-full border-collapse border border-border">
        <tbody>
                ...
                return (
                  <td
                    ...
                    className={cn(
                      ...
                      (!cell.verticalAlignment || cell.verticalAlignment === "top") && "align-top"
                    )}
                    style={{
                    ...
                    onContextMenu={(e) => handleCellContextMenu(rowIndex, cellIndex, e)}
                      ...
                      <div
                        ...
                        onBlur={(e) => {
                          ...
                          if (newText !== para.text) {
                            const newTable = { ...table };
                            newTable.rows[rowIndex].cells[cellIndex].paragraphs[paraIndex].text =
                              newText;
                            onChange(newTable);
                          }
                        }}
