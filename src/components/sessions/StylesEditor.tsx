...
export const StylesEditor = memo(function StylesEditor({
  ...
  const [localPadding1x1Top, setLocalPadding1x1Top] = useState<number>(
    clampPadding(padding1x1Top, 0)
  );
  const [localPadding1x1Bottom, setLocalPadding1x1Bottom] = useState<number>(
    clampPadding(padding1x1Bottom, 0)
  );
  const [localPadding1x1Left, setLocalPadding1x1Left] = useState<number>(
    clampPadding(padding1x1Left, 0.08)
  );
  const [localPadding1x1Right, setLocalPadding1x1Right] = useState<number>(
    clampPadding(padding1x1Right, 0.08)
  );
  const [localPaddingOtherTop, setLocalPaddingOtherTop] = useState<number>(
    clampPadding(paddingOtherTop, 0)
  );
  const [localPaddingOtherBottom, setLocalPaddingOtherBottom] = useState<number>(
    clampPadding(paddingOtherBottom, 0)
  );
  const [localPaddingOtherLeft, setLocalPaddingOtherLeft] = useState<number>(
    clampPadding(paddingOtherLeft, 0.08)
  );
  const [localPaddingOtherRight, setLocalPaddingOtherRight] = useState<number>(
    clampPadding(paddingOtherRight, 0.08)
  );
  const [localCellBorderThickness, setLocalCellBorderThickness] = useState<number>(
  ...
  useEffect(() => {
    ...
    if (paddingOtherBottom !== undefined)
      setLocalPaddingOtherBottom(clampPadding(paddingOtherBottom, 0));
    if (paddingOtherLeft !== undefined)
      setLocalPaddingOtherLeft(clampPadding(paddingOtherLeft, 0.08));
    if (paddingOtherRight !== undefined)
      setLocalPaddingOtherRight(clampPadding(paddingOtherRight, 0.08));
  }, [
    padding1x1Top,
    padding1x1Bottom,
    padding1x1Left,
            ...
            <div>
              ...
              {(style.id === "header1" || style.id === "header2" || style.id === "header3") && (
                <div
                  ...
                  role="group"
                  aria-labelledby={`${style.id}-formatting-label`}
                >
                  <button
  ...
  const renderListBulletSettings = () => {
    return (
      <div className="space-y-4 p-4 border border-border rounded-lg">
        ...
        <div className="space-y-3">
          <h4 className="text-sm font-medium text-muted-foreground">Indentation Settings</h4>
          <p className="text-xs text-muted-foreground">
            Adjust symbol position per level. Text follows with 0.25" hanging indent.
          </p>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs text-muted-foreground">Symbol Position Increment</label>
  ...
  const renderTableShadingSettings = () => {
    ...
    const handleHeader2ShadingChange = (value: string) => {
      ...
      onTableShadingChange?.(
        value,
        localTableOtherShading,
        localImageBorderWidth,
        getCurrentPaddingSettings()
      );
    };
      ...
      <div className="space-y-4 p-4 border border-border rounded-lg">
        ...
        <div className="pt-4 border-t border-border">
          ...
          <p className="text-xs text-muted-foreground mb-4">
            Set the border thickness for all table cells. Borders with color #FFC000 will preserve
            their color.
          </p>
          <div className="flex gap-2 items-center">
            <select
                ...
                <div>
                  ...
                  <div className="flex gap-1 items-center">
                    <input
                      ...
                      onChange={(e) =>
                        handlePaddingChange("padding1x1Top", e.target.value, setLocalPadding1x1Top)
                      }
